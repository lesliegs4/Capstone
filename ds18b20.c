/*
  ...
  WARNING: These routines assume you are using Port D, bit 2 (PD2)
...
// 1-Wire bus pin (ATmega328P DIP pin 4 is PD2)
...
char ds_init(void)
{
    ...
    ONEWIRE_DDR &= ~(1 << ONEWIRE_BIT);        // Put bus in high-Z state
    ONEWIRE_PORT &= ~(1 << ONEWIRE_BIT);       // Put a zero in the port bit
                                // which also turns off internal pullup

    _delay_ms(100);             // Wait for pull-up to take effect
    if ((ONEWIRE_PINR & (1 << ONEWIRE_BIT)) == 0)  // Check that bus is high
...
char ds_reset(void)
{
    ONEWIRE_PORT &= ~(1 << ONEWIRE_BIT); // ensure low when driving
    ONEWIRE_DDR |= (1 << ONEWIRE_BIT);   // Pull bus low
    _delay_us(600);             // Delay >480usec
    ONEWIRE_DDR &= ~(1 << ONEWIRE_BIT);  // Let bus go high (Hi-Z)
    ONEWIRE_PORT &= ~(1 << ONEWIRE_BIT); // keep internal pull-up off
    _delay_us(67);              // Wait for Presense pulse
    if ((ONEWIRE_PINR & (1 << ONEWIRE_BIT)) == 0) {
	while ((ONEWIRE_PINR & (1 << ONEWIRE_BIT)) == 0) {} // Wait for pulse to end
...
void ds_write1bit(void)
{
    ...
    // Write-1 time slot: pull low briefly, then release for remainder.
    ONEWIRE_PORT &= ~(1 << ONEWIRE_BIT);
    ONEWIRE_DDR  |=  (1 << ONEWIRE_BIT);   // drive low
    _delay_us(6);                          // tLOW1 (>=1us, <=15us)
    ONEWIRE_DDR  &= ~(1 << ONEWIRE_BIT);   // release (Hi-Z)
    ONEWIRE_PORT &= ~(1 << ONEWIRE_BIT);   // keep internal pull-up off
    _delay_us(64);                         // finish slot (~60us total)
...
void ds_write0bit(void)
{
    ...
    // See page 15 and 16 of DS18B20 datasheet
    // Write-0 time slot: hold low for most of the slot, then release.
    ONEWIRE_PORT &= ~(1 << ONEWIRE_BIT);
    ONEWIRE_DDR  |=  (1 << ONEWIRE_BIT);   // drive low
    _delay_us(60);                         // tLOW0 (>=60us)
    ONEWIRE_DDR  &= ~(1 << ONEWIRE_BIT);   // release
    ONEWIRE_PORT &= ~(1 << ONEWIRE_BIT);   // keep internal pull-up off
...
unsigned char ds_readbit(void)
{
    ...
    // Read time slot: master pulls low briefly, releases, then samples.
    ONEWIRE_PORT &= ~(1 << ONEWIRE_BIT);
    ONEWIRE_DDR  |=  (1 << ONEWIRE_BIT);   // drive low
    _delay_us(6);                          // tINIT (>=1us)
    ONEWIRE_DDR  &= ~(1 << ONEWIRE_BIT);   // release
    ONEWIRE_PORT &= ~(1 << ONEWIRE_BIT);   // keep internal pull-up off
    _delay_us(9);                          // sample window
    b = (ONEWIRE_PINR & (1 << ONEWIRE_BIT)) ? 1 : 0;
    _delay_us(55);                         // rest of slot
    return b;
